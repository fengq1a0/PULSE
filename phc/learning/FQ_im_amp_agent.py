...
class IMAmpAgent(a2c_continuous.A2CAgent):

    def __init__(self, base_name, config):
        # Call the orginal function directly
        # A2CBase --> ContinuousA2CBase  -->  A2CAgent  --> IMAMPAgent
        ...
        if self.vec_env.env.task.fitting:
            ...
            # TODO: load mean and std here!
            # the shape of mean and std doesn't match!
            # Originally, they load the mean and std, and fix it.
            # But for ours, it should be trainable for the image part
    ...
    def init_tensors(self):
        ...
        self.experience_buffer.tensor_dict['amp_obs'] = torch.zeros(self.experience_buffer.obs_base_shape + self._amp_observation_space.shape, device=self.ppo_device)
        self.tensor_list += ['next_obses']
        self.tensor_list += ['amp_obs']


        self._amp_obs_demo_buffer = replay_buffer.ReplayBuffer(int(self.config['amp_obs_demo_buffer_size']), self.ppo_device)  # Demo is the data from the dataset. Real samples
    ...
    def train_epoch(self):
        self.pre_epoch(self.epoch_num)              # fix running norm, resample motion
        ...
        self._update_amp_demos()                    # update_amp_GT_samples
        ...
        if (self._amp_replay_buffer.get_total_count() == 0):
            batch_dict['amp_obs_replay'] = batch_dict['amp_obs']        # from predicted
        else:
            batch_dict['amp_obs_replay'] = self._amp_replay_buffer.sample(num_obs_samples)['amp_obs'] # from buffered predicted
        ...
        self.post_epoch(self.epoch_num)            # update mean_std for env
    ...
    def train(self):
        ...
        while True:
            ...
            epoch_num = self.update_epoch() # just epoch_num += 1


            train_info = self.train_epoch()


            ######## Log data & save weight ####################
